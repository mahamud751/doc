...
function PatientVideoCallContent() {
  ...
  const cleanup = async () => {
    try {
      ...
      console.error("❌ PATIENT: Cleanup error:", error);
    }
  };
  ...
  const initializeAgora = async () => {
    ...
    try {
      ...
      clientRef.current.on("user-published", async (...args: unknown[]) => {
        ...
        try {
          ...
          if (
            ...
            console.log(
              "✅ PATIENT: DOCTOR VIDEO CONNECTED! Both cameras should now be visible."
            );
          }
      ...
      // Create local tracks
      console.log("📹 PATIENT: Creating local camera and microphone tracks...");
      ...
      if (localVideoTrackRef.current?.play && localVideoRef.current) {
        ...
        console.log(
          "✅ PATIENT: Local camera started - should be visible now!"
        );
      }
      ...
      console.log("✅ PATIENT: Joined channel, waiting for doctor...");
      ...
      console.log("✅ PATIENT: Published tracks");
      ...
      console.error("❌ PATIENT: Initialize error:", error);
  ...
  // 🔥 PROGRESSIVE CONNECTION STATUS: Poll for doctor presence
  useEffect(() => {
    if (!channelName || isConnected) return;

    let pollInterval: NodeJS.Timeout;

    const checkDoctorPresence = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (doctorPresent && patientPresent && !isConnected) {
            console.log(
              "🔥 PATIENT: Both doctor and patient in channel - should connect soon!"
            );
            setCallStatus("connecting"); // Doctor connecting video
          } else if (doctorPresent) {
            console.log(
              "✅ PATIENT: Doctor joined channel - waiting for video connection"
            );
            setCallStatus("connecting");
          } else {
            setCallStatus("calling"); // Still waiting for doctor
          }
        }
      } catch (error) {
        console.log("Error checking doctor presence:", error);
      }
    };

    // Check every 2 seconds
    pollInterval = setInterval(checkDoctorPresence, 2000);
    checkDoctorPresence(); // Check immediately
  ...
  const toggleMute = () => {
    if (localAudioTrackRef.current) {
      ...
      setIsMuted(newMuted);
      console.log(`🔊 PATIENT: Audio ${newMuted ? "muted" : "unmuted"}`);
    }
  };
  ...
  const toggleVideo = () => {
    if (localVideoTrackRef.current) {
      ...
      console.log(`📹 PATIENT: Video ${newVideoOff ? "disabled" : "enabled"}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900 relative">
        ...
        {!isConnected || !remoteUsers.length ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white">
              ...
              {callStatus === "calling" && (
                <>
                  <p className="text-lg">📞 Calling doctor...</p>
              ...
              {callStatus === "connecting" && (
                <>
                  <p className="text-lg">🔗 Doctor connecting video...</p>
                  <p className="text-sm text-gray-300 mt-2">
          ...
          <>
            ...
            <div className="absolute top-4 left-4 bg-blue-500 text-white px-3 py-2 rounded-lg text-base font-semibold shadow-lg">
              👨‍⚕️ Doctor
            </div>
          </>
        )}
      ...
      {/* Local Video (Patient) - Always visible like Messenger */}
      <div className="absolute top-4 right-4 w-2/5 h-2/5 bg-gray-800 rounded-lg border-4 border-green-500 overflow-hidden shadow-2xl z-10">
        {!isVideoOff ? (
          <>
            <div ref={localVideoRef} className="w-full h-full" />
            <div className="absolute bottom-2 left-2 bg-green-500 text-white px-3 py-2 rounded text-sm font-bold shadow-lg">
              🤒 You (Patient)
            </div>
          </>
        ) : (
