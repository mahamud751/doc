...
export default function DebugFullCallFlowPage() {
  ...
  const connectSocket = () => {
    ...
    const handleIncomingCall = (call: ActiveCall) => {
      console.log("=== DEBUG: Received incoming call ===", call);
      addToLog(`=== DEBUG: Received incoming call from ${call.callerName} (${call.callerId}) ===`);
      
      // Always show the call for debugging
      setIncomingCall(call);
      setStatus("Incoming call from " + call.callerName);
      addToLog(`Showing incoming call notification from ${call.callerName}`);
    };

    const handleCallResponse = (response: any) => {
      console.log("=== DEBUG: Received call response ===", response);
      addToLog(`=== DEBUG: Received call response: ${response.accepted ? "accepted" : "rejected"} ===`);
    ...
    const handleCallEnded = (callId: string) => {
      console.log("=== DEBUG: Call ended ===", callId);
      addToLog(`=== DEBUG: Call ended: ${callId} ===`);
  ...
  const handleCall = async () => {
    ...
    setStatus("Calling...");
    addToLog("Call initiated with callId: " + call.callId);
  };

  const handleAccept = () => {
    if (incomingCall) {
      addToLog(`Accepting call ${incomingCall.callId}`);
  ...
  const handleReject = () => {
    if (incomingCall) {
      addToLog(`Rejecting call ${incomingCall.callId}`);
  ...
  const handleEndCall = () => {
    if (outgoingCall) {
      addToLog(`Ending outgoing call ${outgoingCall.callId}`);
      callingService.endCall(outgoingCall.callId);
      setOutgoingCall(null);
      setStatus("Call ended");
      addToLog("Outgoing call ended");
    } else if (incomingCall) {
      addToLog(`Ending incoming call ${incomingCall.callId}`);
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="mt-8 bg-yellow-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold mb-4">Connection Status: {status}</h2>
        ...
        <div className="mt-8 bg-purple-50 p-6 rounded-lg">
          <h2 className="text-xl font-semibold mb-4">Debug Call Log</h2>
