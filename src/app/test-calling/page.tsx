...
export default function TestCallingPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Reset callbacks to null functions instead of empty functions
  ...
  const handleSimulateIncomingCall = () => {
    const callId = `call_${Date.now()}_simulated`;
    const callData: ActiveCall = {
      callId,
      callerId: targetUserId || "simulated-user",
      callerName: targetUserName || "Simulated User",
      calleeId: userId,
      calleeName: userName,
      appointmentId: "simulated-appointment",
      channelName: "simulated-channel",
      status: "ringing",
    };

    // Simulate the incoming call using the calling service method
    callingService.simulateIncomingCall(callData);
    addToLog("Simulated incoming call");
  };
