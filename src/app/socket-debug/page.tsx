...
export default function SocketDebugPage() {
  ...
  const handleSendTestMessage = () => {
    try {
      ...
      addMessage(`Send message error: ${error.message || error}`);
    }
  };
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 h-64 overflow-y-auto">
            {messages.length === 0 ? (
              <p className="text-gray-500">
                No messages yet. Try connecting to the socket server.
              </p>
            ) : (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-yellow-700">
                <strong>Instructions:</strong> Make sure you're running the
                custom server with `node custom-server.js` instead of `npm run
                dev`.
              </p>
            </div>
          </div>
