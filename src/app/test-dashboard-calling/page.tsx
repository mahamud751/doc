...
export default function TestDashboardCallingPage() {
  const [callLog, setCallLog] = useState<string[]>([]);
  const [doctorUserId] = useState("doctor_test_123");
  const [patientUserId] = useState("patient_test_456");
  const [incomingCall, setIncomingCall] = useState<any>(null);
  const [isConnected, setIsConnected] = useState(false);
  const [userMode, setUserMode] = useState<"doctor" | "patient">("patient");
  ...
  }, [userMode, doctorUserId, patientUserId]);
  ...
  const testDashboardCall = async () => {
    ...
    addLog(`📞 ${callerName} calling ${targetName} (Dashboard scenario)...`);
      ...
      const call = await callingService.initiateCall(
        {
          ...
          appointmentId: "test-appointment-dashboard-123",
          channelName: "test-channel-dashboard-123",
        },
        callerId,
        callerName
      );

      addLog(`✅ Dashboard call initiated: ${call.callId}`);
      addLog(`⏳ Waiting for ${targetName} to receive call notification...`);
      addLog(`🎯 Expected: ${targetName} should see incoming call modal immediately!`);
    } catch (error) {
      addLog(`❌ Error initiating dashboard call: ${error}`);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold text-center mb-8">
          🔧 Test Dashboard Calling System Fix
        </h1>
        ...
        {/* Test Controls */}
        <div className="mb-6">
          <h2 className="text-xl font-semibold mb-2">Test Dashboard Call Fix</h2>
          <div className="flex space-x-4">
            <Button
            ...
            >
              📞 Dashboard Call: {userMode === "doctor" ? "Call Patient" : "Call Doctor"}
            </Button>
            <Button
              onClick={clearLog}
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-lg">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>Click "🤒 Patient Mode" to simulate patient dashboard</li>
            <li>Click "📞 Dashboard Call: Call Doctor" to simulate patient calling doctor</li>
            <li>Switch to "👨‍⚕️ Doctor Mode" in another tab</li>
            <li>You should see an incoming call modal immediately on the doctor's side</li>
            <li>Accept or reject the call to test responses</li>
            <li>Test the reverse: doctor calling patient</li>
          </ol>
          <p className="mt-4 text-sm font-semibold text-blue-700">
            🎯 Expected result: Instant bidirectional call notifications between patient and doctor dashboards!
          </p>
          <p className="mt-2 text-sm text-green-700">
            ✅ Fixed: Patient dashboard now uses initiateVideoCall() instead of joinVideoCall()
          </p>
          <p className="mt-1 text-sm text-green-700">
            ✅ Fixed: Doctor dashboard now uses initiateVideoCall() instead of just opening video page
          </p>
        </div>
